...
export default function InteractionsPage(){
  ...
  async function load(){
    ...
    if (ppl && ppl[0] && !personId) setPersonId(ppl[0].id);
          ...
          {rows.map(r => (
            <li key={r.id} className="border rounded p-3">
              <div className="text-sm opacity-70">
                ...
                {' • '}<a className="underline" href={`/people/${r.person_id}`}>{labelOf(r.person_id)}</a>
              </div>
              {r.note ? <p className="mt-1">{r.note}</p> : null}
            </li>
          ))}
          {rows.length === 0 && <li className="text-sm text-gray-500">아직 기록이 없습니다.</li>}
        </ul>
      </section>
    </main>
  );
