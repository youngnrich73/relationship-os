...
export default function IdeasPage(){
  ...
  const ideas = useMemo<Idea[]>(() => {
    ...
    logs.forEach(l => {
      if (!map.has(l.person_id)) map.set(l.person_id, { label: l.person_id, arr: [] as Log[] });
    ...
    for (const [pid, v] of map.entries()){
      ...
      const days = lastAt ? Math.round((now - lastAt) / (1000*60*60*24)) : 999;

      // 평균 기분
      const moods = L.map(x => (typeof x.mood === "number" ? x.mood! : null)).filter(x => x !== null) as number[];
      const moodAvgPct = moods.length ? ((moods.reduce((a,b)=>a+b,0)/moods.length + 3)/6)*100 : 50;
    ...
    res.sort((a,b)=>{
      const lastA = (map.get(a.person_id)!.arr[0]?.happened_at) ?? "1970-01-01";
      const lastB = (map.get(b.person_id)!.arr[0]?.happened_at) ?? "1970-01-01";
      return new Date(lastA).getTime() - new Date(lastB).getTime();
    });
    return res.slice(0, 20); // 최대 20명
  }, [people, logs]);
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <p className="text-sm text-gray-500">최근 기록(90일)을 바탕으로 간단한 규칙으로 제안합니다.</p>
      <div className="grid gap-3">
        {ideas.map((card) => (
          <div key={card.person_id} className="border rounded p-4">
            <div className="font-semibold">{card.label}</div>
            <ul className="mt-2 list-disc pl-5 space-y-1">
              {card.items.map((t, i) => <li key={i}>{t}</li>)}
            </ul>
            <div className="mt-3 text-sm">
              <a className="underline" href={`/people/${card.person_id}`}>타임라인 보기</a>
            </div>
          </div>
        ))}
      </div>
