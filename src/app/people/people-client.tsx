...
export default function PeopleClient() {
  ...
  const add = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("people").insert({
      owner, name, note: note || null,
    } as any);
    if (error) setMsg("추가 오류: " + error.message);
    else {
      setName(""); setNote(""); setMsg("추가 완료!");
      await load();
    }
  };
      ...
      <div className="space-y-2">
        ...
        {list.map(p => (
          <div key={p.id} className="border rounded p-3 flex justify-between items-center">
            ...
            <button onClick={() => remove(p.id)} className="text-sm underline">삭제</button>
          </div>
        ))}
      </div>
